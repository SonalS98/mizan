...
export function renderPublishedEnergy(root: HTMLElement, site: RenewableCase) {
  ...
  const update = () => {
    ...
    output.innerHTML = `<div class="renewable-stats">${series.map(s => `<div><span>${labels[s.source]} · modeled</span><b>${num(s.monthly.reduce((a, b) => a + b, 0) / 1000)} MWh/yr</b></div>`).join("")}<div><span>Combined modeled output</span><b>${num(totals.reduce((a, b) => a + b, 0) / 1000)} MWh/yr</b></div></div>
